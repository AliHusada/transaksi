...
class transaksi_penjualan extends CI_Controller{
	...
	{
		$db['title'] = 'Transaksi';
		echo view('transaksi_penjualan',$db);
	}

	public function read(){
		$transaksi  = $this->db->query('SELECT p.tgl_penjualan as tgl,pl.NAME as nama,p.no_penjualan as nop,p.total as total,p.jumlah as jumlah FROM pembayaran p LEFT JOIN pelanggan pl ON pl.id = p.id_pelanggan where p.no_penjualan like "%JL%" ')->result_array();
		$no = 1;
			foreach ($transaksi as $key ){
				?>
					<tr>
                      ...
                      <td><a href="<?php echo site_url('transaksi_pembelian/show_barang?np=').$key['nop'] ?>">Rincian Barang</a></td>
                    </tr>
